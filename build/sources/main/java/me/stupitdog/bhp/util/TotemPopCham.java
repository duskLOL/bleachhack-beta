...
public class TotemPopCham {
    ...
    public TotemPopCham(EntityOtherPlayerMP player, ModelPlayer playerModel, Long startTime, double alpha) {
        MinecraftForge.EVENT_BUS.register(this);
        this.player = player;
        this.playerModel = playerModel;
        this.startTime = startTime;
        this.alphaFill = alpha;
        this.alphaLine = alpha;
    }
    ...
    public void onRenderWorld(final RenderWorldLastEvent event) {
        ...
        Color color = new Color(Bhp.instance.moduleManager.getModuleByClass(Chams.class).color.getRGB());
        int fillA = (color).getAlpha();
        final long time = System.currentTimeMillis() - this.startTime - ((Number)Bhp.instance.moduleManager.getModuleByClass(Chams.class).fadeStart.getValue()).longValue();
        if (System.currentTimeMillis() - this.startTime > ((Number)Bhp.instance.moduleManager.getModuleByClass(Chams.class).fadeStart.getValue()).longValue()) {
            double normal = this.normalize((double)time, 0.0, ((Number)Bhp.instance.moduleManager.getModuleByClass(Chams.class).fadeSpeed.getValue()).doubleValue());
        ...
        if (this.player != null && this.playerModel != null) {
            ...
            GL11.glEnable(2881);
            GL11.glEnable(2848);
            if (alphaFill > 1) alphaFill -= Bhp.instance.moduleManager.getModuleByClass(Chams.class).fadeSpeed.getValue();
            Color fillFinal = new Color(fillColor.getRed(), fillColor.getGreen(), fillColor.getBlue(), (int)alphaFill);

            if (alphaLine > 1) alphaLine -= Bhp.instance.moduleManager.getModuleByClass(Chams.class).fadeSpeed.getValue();
