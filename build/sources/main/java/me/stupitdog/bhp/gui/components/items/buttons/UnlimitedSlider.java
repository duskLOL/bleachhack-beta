...
public void drawScreen(int mouseX, int mouseY, float partialTicks) {
	RenderUtil.drawRect(this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Bhp.instance.colorManager.getColorWithAlpha(Bhp.instance.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Bhp.instance.colorManager.getColorWithAlpha(Bhp.instance.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
Bhp.instance.fontManager.drawStringWithShadow(" - " + this.setting.getName() + " " + ChatFormatting.GRAY + this.setting.getValue() + ChatFormatting.WHITE + " +", this.x + 2.3f, this.y - 1.7f - (float) BhpGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
}
