...
RenderUtil.drawRect(this.x, this.y, ((Number) this.setting.getValue()).intValue() <= this.min.intValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Bhp.instance.colorManager.getColorWithAlpha(Bhp.instance.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Bhp.instance.colorManager.getColorWithAlpha(Bhp.instance.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
Bhp.instance.fontManager.drawStringWithShadow(this.getName() + " " + ChatFormatting.GRAY + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf(((Number) this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) BhpGui.getClickGui().getTextOffset(), Bhp.instance.moduleManager.getModuleByClass(ClickGui.class).fontColor.getRGB());
}
...
if (this.setting.getValue() instanceof Double) {
    ...
    this.setting.setValue((Integer) this.setting.getMin() + (int) ((int) this.difference * percent));
}
}
