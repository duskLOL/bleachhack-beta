...
if (this.isListening) {
	Bhp.instance.fontManager.drawStringWithShadow(this.currentString.getString() + "_", this.x + 2.3f, this.y - 1.7f - (float) BhpGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
} else {
	Bhp.instance.fontManager.drawStringWithShadow((this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.GRAY : "")) + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) BhpGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
}
}
